...
export default function Navbar() {
  ...
  const linkClass = (path: string, isMobile: boolean = false) => {
    const baseClass = isMobile 
      ? "block w-full text-left px-4 py-3 text-lg rounded-xl transition-all" 
            ...
            {isPackagesDropdownOpen && (
              ...
              >
                <div className="flex flex-col gap-1.5">
                  <button 
                    onClick={() => { setTimeout(() => { navigate("/group-packages"); setIsPackagesDropdownOpen(false); }, 1000); }} 
                    className="block px-5 py-4 rounded-2xl text-gray-800 font-bold hover:bg-[#FFC107] hover:text-black transition-all text-sm text-left w-full"
                  >
                    Group / Family Packages
                  </button>
                  <button 
                    onClick={() => { setTimeout(() => { navigate("/packages"); setIsPackagesDropdownOpen(false); }, 1000); }} 
                    className="block px-5 py-4 rounded-2xl text-gray-800 font-bold hover:bg-[#FFC107] hover:text-black transition-all text-sm text-left w-full"
                  >
                    Popular Packages
                  </button>
          ...
          >
            <div className="flex flex-col py-4 px-6 gap-2">
              ...
              <div className="bg-gray-50 border-y border-gray-100 py-4 my-2 rounded-2xl">
                <button 
                  className="block w-full text-left px-6 py-3 text-gray-700 font-bold hover:bg-[#FFC107] hover:text-black rounded-lg transition-all" 
                  onClick={() => { setTimeout(() => { navigate("/group-packages"); setIsMobileMenuOpen(false); }, 1000); }}
                >
                  Group / Family Packages
                </button>
                <button 
                  className="block w-full text-left px-6 py-3 text-gray-700 font-bold hover:bg-[#FFC107] hover:text-black rounded-lg transition-all" 
                  onClick={() => { setTimeout(() => { navigate("/packages"); setIsMobileMenuOpen(false); }, 1000); }}
                >
                  Popular Packages
                </button>
              </div>
